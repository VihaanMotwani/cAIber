    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <h2 className="text-2xl font-orbitron text-gray-400">No Threat Model Available</h2>
  ...
  return (
    <div className="space-y-8">
          ...
          {attackPaths.map((path, idx) => (
            <button
              ...
              className={`w-full text-left p-4 rounded-lg border transition-all duration-200 ${
                selectedPath === idx 
                  ? 'bg-cyber-red/20 border-cyber-red shadow-lg shadow-cyber-red/30' 
                  : 'bg-cyber-dark/30 border-gray-700 hover:border-cyber-red/50'
              }`}
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-mono text-sm text-cyber-yellow">Scenario #{idx + 1}</p>
                ...
                <ChevronRight className={`w-5 h-5 transition-transform ${
                  selectedPath === idx ? 'rotate-90 text-cyber-red' : 'text-gray-500'
                }`} />
              </div>
            </button>
          ))}
      ...
      <motion.div
        ...
        className="bg-cyber-light/30 backdrop-blur-md rounded-lg p-6 border border-cyber-purple/30"
      >
        <h2 className="text-xl font-orbitron text-cyber-purple mb-4 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {Array.from(new Set(
            ...
            <div key={idx} className="bg-cyber-dark/50 rounded-lg p-3 border border-cyber-purple/20">
              <p className="text-xs text-cyber-purple font-mono uppercase">{tactic}</p>
      ...
      <motion.div
        ...
        className="bg-cyber-light/30 backdrop-blur-md rounded-lg p-6 border border-cyber-cyan/30"
      >
        <h2 className="text-xl font-orbitron text-cyber-cyan mb-4 flex items-center gap-2">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {['Spoofing', 'Tampering', 'Repudiation', 'Information Disclosure', 'Denial of Service', 'Elevation of Privilege'].map((stride) => {
            ...
            return (
              <div key={stride} className="bg-cyber-dark/50 rounded-lg p-3 border border-cyber-cyan/20">
                <p className="text-xs text-cyber-cyan font-mono">{stride}</p>
